 ...
 */
	...
	var sizeContent = function (panes, remeasure) {
		...
		$.each(panes.split(","), function (idx, pane) {
			...
			var newH = s.innerHeight - m.top - m.footersHeight;
			...
			function _measure () {
				...
				,	footersHeight:	0 // correct if no content footer ($E)
				}
					m.headersHeight	= m.top; // just for state - not used in calc
					m.bottom		= m.top + m.height;
				if ($F.length)
					m.footersHeight = ($F[0].offsetTop - m.bottom) + $F.outerHeight() + (parseInt($F.css("marginBottom")) || 0);
					//footersHeight = LastFooter.top - Content.bottom + LastFooter.height + LastFooter.marginBottom
			};
		});
	};
