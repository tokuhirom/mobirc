...
sub new {
    ...
    croak "global context missing" unless blessed $global_context && $global_context->isa("Mobirc");
...
sub add_message {
    ...
    if ($message->class eq 'public' && $self->name ne '*keyword*') {
        if (any { $message->body =~ /$_/i } @{$self->{global_context}->config->{global}->{keywords} || []}) {
            Mobirc::Channel->update_keyword_buffer($self->{global_context}, $message);
        }
    }
}
